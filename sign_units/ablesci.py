...
def ablesci_sign(browser, cookie_file, blank_window, rank, total, temp_file=sys.stdout):
    rank += 1
    ...
    try:
        print("开始 科研通 签到", file=temp_file)
        browser.switch_to.new_window("ablesci")
        browser.get("https://www.ablesci.com/")
    ...
    try:
        sign_button = browser.find_elements(by=By.XPATH, value='''//div[@class="fly-panel-main"]/div[@class="fly-signin"]//button''')
        if not sign_button:
